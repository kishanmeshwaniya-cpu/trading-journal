...
st.markdown("""
    ...
    /* Matrix Main Wrapper (The White Box) */
    .matrix-container {
...
if uploaded_files:
    ...
    for f in uploaded_files:
        ...
        if 'Realised P&L' in df.columns: # Delta
            ...
            df['Trade_Time'] = pd.to_datetime(df['Time'].astype(str).str[:19])
            df['Asset'] = df['Contract']
            all_data.append(df[['Date', 'Trade_Time', 'Asset', 'P&L']])
            ...
            df['Trade_Time'] = pd.to_datetime(df['Time'], format='mixed')
            all_data.append(df[['Date', 'Trade_Time', 'Name', 'P&L']].rename(columns={'Name':'Asset'}))

    if all_data:
        final_df = pd.concat(all_data)
        final_df['Category'] = final_df['Asset'].apply(categorize_asset)
        ...
        fig_eq = px.area(daily, x='Date', y='Equity', title="Total Portfolio Growth")
        ...
        with c_g2:
            final_df['Hour'] = final_df['Trade_Time'].dt.hour
