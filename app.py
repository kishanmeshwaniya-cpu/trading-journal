...
if uploaded_files:
    ...
    # ---------------------------------------------
    # 3. SELF-EVOLVING AI ENGINE (NOW WITH TABLES)
    # ---------------------------------------------
    st.markdown("---")
    st.subheader("🧠 Gemini Core: Auto-Evolving Strategy")
    st.write("Gemini exact data scan karega aur aapko ek structured table mein improvement areas batayega.")
    
    if st.button("Initialize Deep Scan"):
        if model is not None and not analytics_df.empty:
            with st.spinner("Compiling visual data reports..."):
                try:
                    ai_feed = analytics_df[['Date_Clean', 'Trade_Time', 'Asset', 'P&L_Clean']].tail(50).to_string(index=False)
                    
                    prompt = f"""
                    Tu ek elite Quant Analyst hai.
                    Trader ke is chronological data ko padh aur usme se HIDDEN PATTERNS nikal (jaise revenge trading, worst trading hour, ya best asset).
                    
                    Apne response ko STRICTLY ek clear, visual Markdown Table mein present kar. Koi lamba essay ya paragraph nahi likhna. Use this exact structure:
                    
                    ### 🕵️‍♂️ Pattern Analysis Dashboard
                    | Factor (Time/Asset) | Data Observation (What happened?) | Status |
                    |---|---|---|
                    | (e.g., 11:00 AM) | Loss ke turant baad dusri entry li gayi | 🔴 Warning |
                    | (e.g., NIFTY 24450) | Is asset mein sabse zyada consistency hai | 🟢 Strength |
                    *(Add 3-4 highly specific rows based on actual data)*
                    
                    ### ⚙️ System Upgrade Rule
                    (Sirf 1 single, strict algorithmic rule de jo trader ko kal follow karni hai apne drawdowns rokne ke liye. Hinglish mein.)
